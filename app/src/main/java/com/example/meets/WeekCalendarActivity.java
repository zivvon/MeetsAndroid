...
import com.example.meets.Adapter.WeekCalendarAdapter;
...
public class WeekCalendarActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Cal 버튼 설정
        moveMonthCal = findViewById(R.id.up_down_btn);
        ...
        //개인 일정 다이얼로그
        todo_dialog = new Dialog(WeekCalendarActivity.this);
    ...
    public void moveMonthCal(View v){
        Intent intent = new Intent(getApplicationContext(), CalendarActivity.class);
        startActivity(intent);
    }
    ...
    private void setMonthView() {
        ...
        //어댑터 데이터 적용
        WeekCalendarAdapter adapter = new WeekCalendarAdapter(dayList);
...
//        int dayOfWeek = firstDay.getDayOfWeek().getValue();
        ...
        int selectedDay = CalendarUtil.selectedDate.getDayOfWeek().getValue() - 1;

        for (int i = 0; i < 7; i++){
            dayList.add(LocalDate.of(CalendarUtil.selectedDate.getYear(), CalendarUtil.selectedDate.getMonth(),
                    CalendarUtil.selectedDate.getDayOfMonth() - selectedDay + i));
        }
